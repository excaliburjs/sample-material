...
void main() {
  ...
  vec2 pixelCoords = (v_uv - 0.5) * u_graphic_resolution;
  ...
  float distanceFalloff = 1.0 / (d / 1000.0);
	...
	if (d <= 0.0) {
		...
		fragColor = vec4(drawStar(pixelCoords, u_color.rgb, starSize, d) * u_opacity, step(0.0, -d - 1.0));
		fragColor.a = u_opacity * fragColor.a;

	} else if (d <= starSize * 3.0 && u_highlight) {
     vec3 newCol = u_color.rgb * distanceFalloff;
     //(0.6*distanceFalloff)
